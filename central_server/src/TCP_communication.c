...
int people_first_floor = 0;
int people_building = 0;
int people_ground = 0;
...
void *read_from_distributed_server(void *args){
    ...
    strcpy(central_server_address, "192.168.0.53");
    central_server_address[13] = '\0';
    ...
    server_addr.sin_addr.s_addr = inet_addr(central_server_address);
    server_addr.sin_port = htons(central_server_port);

    printf("Central server addr = %s:%d\n", central_server_address, central_server_port);
    ...
    while(1){
        ...
        if ((length_msg = recv(client_socket, buffer, 100, 0)) < 0)
...
void *write_to_distributed_server(void *args){
    ...
    server_info *my_server_info = (server_info *) malloc(sizeof(server_info));
    strcpy(my_server_info->ip_addr, "192.168.0.52");
    my_server_info->port = 10234;
...
void handle_received_messages(int msg_code){
    if (msg_code >= 10 && msg_code <= 40){
        if (msg_code == 10){
            (people_building == 0) ? people_building = 0 : people_building--;
	        (people_ground == 0) ? people_ground = 0 : people_ground--;
        } else if (msg_code == 20){
            people_building++;
	        people_ground++;
        } else if (msg_code == 30){
            (people_first_floor == 0) ? people_first_floor = 0 : people_first_floor--;
	        people_ground++;
        } else if (msg_code == 40){
            (people_ground == 0) ? people_ground = 0 : people_ground--;
	        people_first_floor++;
        }
        printf("Prédio = %d, Térreo = %d, 1º Andar = %d\n", people_building, people_ground, people_first_floor);
    }
}
